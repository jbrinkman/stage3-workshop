#!/usr/bin/env node
// Combined Kiro CLI provider and grader for promptfoo
//
// Auto-detects mode based on prompt format:
// - Grader mode: Prompt is JSON array [{role, content}, ...]
// - Provider mode: Prompt is plain text
//
// Grader: Parses JSON chat array, concatenates system + user messages (no --system-prompt flag)
// Provider: Passes prompt directly to Kiro CLI
//
// Requires KIRO_API_KEY environment variable for headless operation.
...
let model = 'claude-sonnet-4.6'; // Default model
if (options && options !== '{}') {
  try {
...
if (isGraderMode) {
  ...
  // Combine system and user into one prompt (kiro-cli has no --system-prompt flag)
  const fullPrompt = `${systemMsg}\n\n${userMsg}`;

  // Build command with optional model flag
  const args = ['chat', '--no-interactive', fullPrompt];
  if (model) {
